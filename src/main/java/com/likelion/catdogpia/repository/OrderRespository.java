...
public interface OrderRespository extends JpaRepository<Orders, Long> {
    ...
    // 마이페이지 - 주문 내역 조회
    @Query("SELECT NEW com.likelion.catdogpia.domain.dto.mypage.OrderListDto(o.id, o.orderAt, op.quantity, op.orderStatus, po.size, po.color, p.name, p.price, op.quantity * p.price) " +
            "FROM Orders o " +
